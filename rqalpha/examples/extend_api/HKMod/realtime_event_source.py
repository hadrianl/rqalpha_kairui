#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2018/10/30 0030 9:34
# @Author  : Hadrianl 
# @File    : realtime_event_source
...
class RealtimeEventSource(AbstractEventSource):
    ...
    def clock_worker(self):
        ...
        while True:
            ...
            if d.strftime("%H:%M:%S") >= "08:45:00" and d.date() > self.before_trading_fire_date:
                self.event_queue.put((d, EVENT.BEFORE_TRADING))
                self.before_trading_fire_date = d.date()
            elif d.strftime("%H:%M:%S") >= "16:45:00" and d.date() > self.after_trading_fire_date:
                self.event_queue.put((d, EVENT.AFTER_TRADING))
                self.after_trading_fire_date = d.date()
            elif d.strftime("%H:%M:%S") >= "17:00:00" and d.date() > self.settlement_fire_date:
    ...
    def events(self, start_date, end_date, frequency):
        ...
        while running:
            ...
            while True:
                try:
                    trade_dt, event_type = self.event_queue.get(timeout=1)
                    # print(trade_dt, event_type)
                    break
                except Empty:
                    continue

            system_log.debug("real_dt {}, dt {}, event {}", real_dt, trade_dt, event_type)
            yield Event(event_type, calendar_dt=trade_dt, trading_dt=trade_dt)
