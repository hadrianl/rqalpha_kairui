...
class Scheduler(object):
    ...
    def set_trading_dates_(cls, trading_dates):
        trading_dates = np.apply_along_axis(lambda x: [i.date() for i in x], 0, trading_dates)
    ...
    def _should_trigger(self, n):
        # 非股票交易时间段不触发
        if self._current_minute < 9*60+16 or self._current_minute > 16*60 + 29:
            return False
    ...
    def _fill_week(self):
        ...
        self._this_week = [d for d in self._TRADING_DATES[left:right]]
    ...
    def _fill_month(self):
        ...
        self._this_month = [d for d in self._TRADING_DATES[left:right]]
